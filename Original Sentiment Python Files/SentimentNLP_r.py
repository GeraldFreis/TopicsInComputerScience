...
tokenized_all_x = tokenizer.texts_to_sequences(raw_x)
...
padded_all_x = tensorflow.keras.utils.pad_sequences(tokenized_all_x, padding="post", maxlen=1000)
import keras
...
prediction_training = CNN.predict(padded_all_x, verbose=1, workers=1)
for i in range(0, len(prediction_training)):
    if(prediction_training[i] >= 0.9):
